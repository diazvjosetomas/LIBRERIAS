...
switch ($accion) 
{
	...
	case 'busca_paciente':
		$sql=mysqli_query($enlace,"SELECT * FROM pacientes WHERE cedula=$filtro_paciente ") or die ("Error: ".mysqli_error($enlace));
		$nfilas=mysqli_num_rows($sql);
		$datos=mysqli_fetch_array($sql);

		if($nfilas>0)
			echo json_encode(array('result' => true,'cedula'=>$datos["cedula"],'nombres'=>utf8_encode($datos["nombres"]),"apellidos"=>utf8_encode($datos["apellidos"])));			
		else
