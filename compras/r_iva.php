				...
				<label class="col-sm-3 control-label"><b>Proveedor: </b></label>
					<div class="col-sm-3">
						...
						while ($rs = mysqli_fetch_assoc($result1)) {
							?>
							<option value="<?= $rs["idproveedor"] ?>"><?= utf8_encode($rs["nombre"]) ?></option>
							<?php
						}
						?>
						</select>	
...
function buscarFacturas()
{
	...
	if(tiposalida==""){
		crear_dialog("Alerta","Seleccione el tipo de salida.","tiposalida");
		return false;	
	}

	if(idproveedor==""){
		crear_dialog("Alerta","Seleccione un proveedor.","idproveedor");
		return false;	
	}

	$("#divDatos").load("compras/r_iva_datos.php",{
		"accion":"verFacturas",
...
$('#myModal').modal('show');
$.get("compras/r_iva_datos.php",{accion:"buscarDetalle",idfactura:idfactura},function(result){
